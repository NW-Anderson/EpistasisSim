...
registerDoMC(7)
setwd("/media/lee/HDD_Array/nwanderson/EpistasisSim/jacccalc/nlocisnps/")
...
# setwd("/Volumes/T7/EpistasisSim/nlocisnps")
...
sim.results[3,] <- rep(c(round(seq(from = 10, to = 4970, length.out = 12)), 4977), each = 12, times = 1)
for(ff in FFs){
  files <- list.files(path = paste("./SLiMouts/", ff, "/", sep = ''))
  splt <- do.call(rbind, strsplit(files, "_"))
  for(nloci in c(round(seq(from = 10, to = 4970, length.out = 12)), 4977)){
    ...
    if(nrow(meanjaccs) != 100) meanjaccs <- rbind(meanjaccs, array(dim = c(5,2)))
